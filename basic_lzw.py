# for the tif files that were LZW compressed
...
layout = [
    ...
    [sg.InputText(key='-output name-'),sg.FolderBrowse(target='-output name-')],
    ...
    [sg.Text('Pixel size:', size =(fs_w, fs_h)), sg.InputText(key='-pixel size-',default_text='2.5')],
    [sg.Text('Pixel size unit:', size =(fs_w, fs_h)),sg.Combo(['m','µm','nm'],default_value='nm',key='-pixel size unit-')],
...
while True:
    ...
    # Autofill
        ...
        #with tf.TiffFile(input) as tif:
        #    for page in tif.pages:
        #         for tag in page.tags:
        #            tag_name, tag_value = tag.name, tag.value
        #            if tag_name != "StripByteCounts" and tag_name != "StripOffsets":
        ...
        with tf.TiffFile(filename) as tif:
            for page in tif.pages:
                for tag in page.tags:
                    tag_name, tag_value = tag.name, tag.value
                    if tag_name != "TileOffsets" and tag_name != "TileByteCounts":
